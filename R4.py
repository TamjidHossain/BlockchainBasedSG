...
index_value_dict = {71:0.99, 72:0.62, 73:-100, 74:-35.0, 81:1.0, 82:3.80, 83:0.0, 84:0.0,
...
while 1:
    ...
    if (count%8) == 0:
        index_value_dict[84] = 250
    else:
        index_value_dict[84] = 0.0
        
    if(len(relay_req_pack) == 14):
        ...
        if "Mining Data=" in dataPack1:
            ...
            final_consumption_val.append(['owner', 'R4'])
            ...
            ## Own blockchain copy
            blockchain_file1 = open("/home/user/Desktop/BlockchainBasedSG/blockchain_R4.txt","r")
            blockchain_file_data1 = blockchain_file1.read()
            dataPack5 = str(blockchain_file_data1)
            dataPack5 = dataPack5.replace("'", '"')
            res5 = json.loads(dataPack5)            ##res5 is for R4

            if (len(res5) > 10):
                filename_index = str(res5[-1]['index'])[0:-1]
                filename = "/home/user/Desktop/BlockchainBasedSG/blockchain"+ str(filename_index)+"_R4.zip"
                file_zip = zipfile.ZipFile(filename, 'w')
                file_zip.write('/home/user/Desktop/BlockchainBasedSG/blockchain_R4.txt', compress_type=zipfile.ZIP_DEFLATED)
                 
                file_zip.close()
                
                res12 = []
                res12.append(res5[-1])
                text_file = open('/home/user/Desktop/BlockchainBasedSG/blockchain_R4.txt', "wt")
            ...
            if "Updated Blockchain = " in dataPack1:
                ## Own blockchain copy
                blockchain_file = open("/home/user/Desktop/BlockchainBasedSG/blockchain_R4.txt","r")
                blockchain_file_data = blockchain_file.read()
                dataPack2 = str(blockchain_file_data)
                dataPack2 = dataPack2.replace("'", '"')
                res2 = json.loads(dataPack2)            ##res2 is for R4
                ...
                dataPack5 = str(blockchain_file_data1)
                dataPack5 = dataPack5.replace("'", '"')
                res5 = json.loads(dataPack5)            ##res2 is for R4
                ...
                if len(res1) < len(res2):               ## if DA has less entry than R4    
                    if(res1[-1] == res2[-1]):
