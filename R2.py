...
index_value_dict = {31:1.0, 32:4.77, 33:85.0, 34:-3.65, 41:0.99, 42:-2.41, 43:0.0, 44:0.0}
...
while 1:
    ...
    # Below mentioned 10 is the length of request pack coming from DA. 2 bytes for BB and rest 8 bytes for measurement indices (31,32,33,34,41,42,43,44)
    if(len(relay_req_pack) == 10):
        ...
        if "Mining Data=" in dataPack1:
            ...
            final_consumption_val.append(['owner', 'R2'])
            ...
            '''
            [['consumption_data',[['P',14.439], ['Q', -3.347], ['Vm',0.992], ['Vp',1.112]]], ['owner','R2'], ['recipient','CC']]
            
            '''
                
            blockchain_script = "/home/user/Desktop/BlockchainBasedSG/blockchain_code.py"
            ...
            ## Own blockchain copy
            blockchain_file1 = open("/home/user/Desktop/BlockchainBasedSG/blockchain_R2.txt","r")
            blockchain_file_data1 = blockchain_file1.read()
            dataPack5 = str(blockchain_file_data1)
            dataPack5 = dataPack5.replace("'", '"')
            res5 = json.loads(dataPack5)            ##res5 is for R2
            
          
            if (len(res5) > 10):
                filename_index = str(res5[-1]['index'])[0:-1]
                filename = "/home/user/Desktop/BlockchainBasedSG/blockchain"+ str(filename_index)+"_R2.zip"
                file_zip = zipfile.ZipFile(filename, 'w')
                file_zip.write('/home/user/Desktop/BlockchainBasedSG/blockchain_R2.txt', compress_type=zipfile.ZIP_DEFLATED)
                 
                file_zip.close()
                
                res12 = []
                res12.append(res5[-1])
                text_file = open('/home/user/Desktop/BlockchainBasedSG/blockchain_R2.txt', "wt")
            ...
            if "Updated Blockchain = " in dataPack1:
                ## Own blockchain copy
                blockchain_file = open("/home/user/Desktop/BlockchainBasedSG/blockchain_R2.txt","r")
                blockchain_file_data = blockchain_file.read()
                dataPack2 = str(blockchain_file_data)
                dataPack2 = dataPack2.replace("'", '"')
                res2 = json.loads(dataPack2)            ##res2 is for R2
                ...
                if len(res1) < len(res2):               ## if DA has less entry than R2
